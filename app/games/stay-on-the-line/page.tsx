...
export default function StayOnTheLine() {
  ...
  const dotPositionRef = useRef<Point>({
    x: CANVAS_WIDTH / 2,
    y: CANVAS_HEIGHT - 50,
  });
  ...
  const gameLoop = useCallback(() => {
    ...
    // Check collision
    const currentPathSegment = pathPointsRef.current.find(
      (point) => point.y >= dotPositionRef.current.y
    );
    if (currentPathSegment) {
      const distance = Math.abs(
        currentPathSegment.x - dotPositionRef.current.x
      );
  ...
  const handleTap = (e: React.MouseEvent<HTMLCanvasElement>) => {
    ...
    if (clickX < canvasMiddle) {
      // Move left
      dotPositionRef.current.x = Math.max(
        DOT_RADIUS,
        dotPositionRef.current.x - DOT_MOVE_SPEED
      );
    } else {
      // Move right
      dotPositionRef.current.x = Math.min(
        CANVAS_WIDTH - DOT_RADIUS,
        dotPositionRef.current.x + DOT_MOVE_SPEED
      );
    }
  };
