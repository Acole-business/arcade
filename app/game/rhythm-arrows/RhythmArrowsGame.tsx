...
export default function RhythmArrowsGame({ onScoreUpdate }: Props) {
  ...
  useEffect(() => {
    ...
    const gameLoop = setInterval(() => {
      ...
      setArrows((prevArrows) => {
        ...
        const remainingArrows = updatedArrows.filter(
          (arrow) => arrow.y <= CANVAS_HEIGHT
        );

        // Check for missed arrows
        const missedArrows = updatedArrows.filter(
          (arrow) => arrow.y > TARGET_Y + ARROW_SIZE
        );
      ...
      if (direction) {
        const hitArrow = arrows.find(
          (arrow) =>
            ...
            Math.abs(arrow.y - TARGET_Y) < ARROW_SIZE / 2
        );

        if (hitArrow) {
          setArrows((prevArrows) =>
            prevArrows.filter((arrow) => arrow !== hitArrow)
          );
          setScore((prevScore) => {
  ...
  const playSound = (type: "hit" | "miss") => {
    const audio = new Audio(
      type === "hit" ? "/sounds/hit.mp3" : "/sounds/miss.mp3"
    );
    audio.play();
  };
