...
export default function EmojiMemoryChallenge() {
  ...
  const endGame = (finalScore: number) => {
    ...
    if (finalScore > highScore) {
      ...
      localStorage.setItem("emojiMemoryHighScore", finalScore.toString())
    }
  }
        ...
        <div className="terminal-container flex-1 border border-green-500 rounded-md p-4">
          <div className="h-full flex flex-col items-center justify-center">
            <div className="text-center">
              <p className="mb-4">Loading Emoji Memory Challenge...</p>
  ...
  return (
    <div className="min-h-screen bg-black text-green-500 font-mono p-4 flex flex-col">
      <div className="terminal-container flex-1 border border-green-500 rounded-md p-4 relative">
        ...
        <div className="game-container space-y-6">
          <div className="game-info text-center">
            <h1 className="text-2xl font-bold mb-2">Emoji Memory Challenge 😃➡️🤔</h1>
            <p className="text-sm mb-4">Memorize the emoji sequence and recreate it!</p>
          ...
          {gameStarted && !gameOver && <EmojiMemoryGame onGameOver={endGame} onScoreUpdate={setScore} />}
      ...
      <div className="mt-4 text-xs text-center text-green-700">
        <p>Memorize the emoji sequence, then click the emojis in the correct order!</p>
      </div>
    </div>
  )
}
